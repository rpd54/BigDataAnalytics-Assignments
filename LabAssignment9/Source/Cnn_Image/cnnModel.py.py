...
labelGlob = glob.glob("dataset/train/*")
labelMaster = np.ndarray(shape=(4), dtype=np.dtype('S20'))
...
testFiles = glob.glob("dataset/test/*/*.jpg")
...
allImages = np.ndarray(shape=(201,40000))
allLabels = np.ndarray(shape=(201,4))

testImages = np.ndarray(shape=(99,40000))
testLabels = np.ndarray(shape=(99,4))
...
for i in range(100):
    ...
    if i % 25 == 0:
        ...
        train_r=train_accuracy


#print("Final Accuracy: %g" % accuracy.eval(feed_dict={x: testImages, y_: testLabels, keep_prob: 1.0}))

print("Accurcay %g"%(train_r))
...
print("End Time: ", endModelTime)
print("Total Time:       ", endModelTime-startTime)